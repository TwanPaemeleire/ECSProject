...
namespace Bloodforge
{
	...
	void AudioSourceSystem::SetAudioOfTrack(AudioSource& audioSource)
	{
		MIX_SetTrackAudio(m_AllMixTracks[audioSource.TrackIndex], ResourceManager::GetInstance().GetAudio(audioSource.CurrentSoundId));
	}

	void AudioSourceSystem::StartPlayingAudioSource(AudioSource& audioSource)
	{
		MIX_PlayTrack(m_AllMixTracks[audioSource.TrackIndex], 0);
	}

	void AudioSourceSystem::StopPlayingAudioSource(AudioSource& audioSource)
	{
		MIX_StopTrack(m_AllMixTracks[audioSource.TrackIndex], 0);
	}

	void AudioSourceSystem::PauseAudioSource(AudioSource& audioSource)
	{
		MIX_PauseTrack(m_AllMixTracks[audioSource.TrackIndex]);
	}

	void AudioSourceSystem::ResumeAudioSource(AudioSource& audioSource)
	{
		MIX_ResumeTrack(m_AllMixTracks[audioSource.TrackIndex]);
	}

	void AudioSourceSystem::AddAudioSource(AudioSource& audioSource)
	{
		audioSource.TrackIndex = GetAvailableMixTrackIndex();
	}

	void AudioSourceSystem::RemoveAudioSource(AudioSource& audioSource)
	{
		audioSource.Stop();
